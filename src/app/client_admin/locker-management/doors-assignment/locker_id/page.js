...
const LockerInformationDetails = ({ locker_id = "1" }) => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          device_id: editedData.device_id,
          updated_at: new Date(),
        })
        .eq('id', locker_id);
      if (error) throw error;
  ...
  return (
    <div className="min-h-screen  p-4 md:p-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        <Package className="absolute bottom-20 right-1/4 w-6 h-6 text-orange-300 opacity-20 animate-pulse" style={{animationDelay: '1s'}} />
      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <Input
                            ...
                            onChange={handleInputChange}
                            className="border-gray-300 focus:ring-2 focus:ring-orange-500 focus:border-transparent rounded-xl flex-1"
                            placeholder="e.g., #FF6B35 or Orange"
                          />
                          {editedData.color && (
                            <div 
                              className="w-12 h-10 rounded-xl border-2 border-gray-300 shadow-inner"
                              style={{ 
                                backgroundColor: editedData.color.startsWith('#') ? editedData.color : undefined,
                                background: editedData.color.startsWith('#') ? editedData.color : `linear-gradient(45deg, ${editedData.color}, ${editedData.color})`
                              }}
                            />
                          )}
                        </div>
                            ...
                            {devices
                              ...
                              .map(device => (
                                <option key={device.id} value={device.id}>
                                  {device.device_id} - {device.manufacturer} {device.model} ({device.clients?.name || 'No client'})
                                </option>
                              ))
                            }
                          </select>
                        ...
                        <div className="bg-gradient-to-r from-orange-50 to-gray-50 rounded-xl p-4 border border-orange-200">
                          ...
                          <div className="mt-3 pt-3 border-t border-orange-300">
                            <div className="flex items-center justify-between">
                              ...
                              <Badge className={`${selectedDevice.status === 'active' ? 'bg-green-500' : 'bg-red-500'} text-white`}>
              ...
              <CardContent className="pt-6">
                <div className="space-y-3">
                  <Button 
                    ...
                    onClick={() => {/* Add view usage history logic */}}
                  >
                  ...
                  <Button 
                    variant="outline"
                    className="w-full border-gray-300 hover:border-red-500 text-gray-700 hover:text-red-600 justify-start"
                    onClick={() => {/* Add delete locker logic */}}
                  >
                    <X className="w-4 h-4 mr-2" />
                    Delete Locker
                  </Button>
